  ...
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \centering
        \only<1>{%
          \resizebox{.9\linewidth}{!}{\input{figures/thesis/fids/year_histogram.pgf}}
          \caption{Evolution of the number of publications on FIDSs (until 2024-04).}    
        }
        \only<2->{%
          \resizebox{\linewidth}{!}{\input{figures/thesis/fids/domain_pie.pgf}}
...
\begin{frame}{Federated Intrusion Detection Systems (FIDSs)}
  ...
  \only<2->{\footcite*{lavaur_tnsm_2022}}
...
%   \only<3->{\footcite*{lavaur_icdcs_demo_2024}}
% \end{frame}